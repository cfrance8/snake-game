...
// ── Game helpers ────────────────────────────────────────
function placeFood(exclude, obstacles, cols, rows) {
  let pos, tries = 0;
  do {
...
function startGame(room) {
  ...
  const allBodies = room.snakes.filter(s => s).flatMap(s => s.body);
  const { cols: c, rows: rr } = room.settings;
  room.foods = [
    placeFood(allBodies, room.obstacles, c, rr),
    placeFood(allBodies, room.obstacles, c, rr)
  ];
...
function stepGame(room) {
  ...
  const allBodies = survivors.flatMap(s => s.body);
  eatenIdxs.forEach(idx => {
    room.foods[idx] = placeFood(
      [...allBodies, ...room.foods.filter((_, i) => i !== idx && room.foods[i])],
      room.obstacles, cols, rows
    );
  });
...
function broadcastState(room) {
  broadcast(room, {
    type: 'state',
    snakes: room.snakes.map(s => s ? { id: s.id, body: s.body, dir: s.dir, alive: s.alive, color: s.color } : null),
    foods: room.foods,
    ...
    switch (msg.type) {
      case 'create': {
        ...
        room = {
          ...
          settings: {
            cols: boardSz.cols, rows: boardSz.rows,
      ...
      case 'restart': {
        ...
        if (room.restartVotes.size >= needed) {
          ...
          broadcast(room, { type: 'restart_vote', from: playerIdx + 1, have: room.restartVotes.size, need: needed });
        }
        break;
      }
...
server.listen(5000, '0.0.0.0', () => {
  console.log('Snake server running on port 5000');
});
